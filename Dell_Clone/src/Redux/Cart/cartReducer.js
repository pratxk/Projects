...
export const cartReducer = (state = initialState, { type, payload }) => {
    switch (type) {
        ...
        case DELETE_CART_PRODUCTS_SUCCESS:
            return { ...state, isLoading: false, isError: false, products: state.products.filter(product => product.id !== payload) };
        ...
        case CLEAR_CART_PRODUCTS_SUCCESS:
            return { ...state, isLoading: false, isError: false, products: [] };
        ...
        case UPDATE_CART_ITEMS_PRODUCTS_SUCCESS:
            return { ...state, isLoading: false, isError: false, products: state.products.map(product => product.id === payload.id ? { ...product, items: payload.items } : product) };
